...
def train(model, gan, train_dataset, train_dataloader, device, optimizer, optimizer_gan, criterion, criterion_gan, a, b, c, d):
    ...
    counter = 0
    display_interval = 100 ### For visualization
    eval_frequency = 100 ### Evaluate LPIPS and FID metrics
    ...
    for i, data in enumerate(prog_bar):
        ...
        input_occluded = data[0].to(device)  # occluded image
        target = data[1].to(device)          # original face
        inpainted_image = model(input_occluded)
        ...
        perceptual,style, content = criterion(inpainted_image,target)
        ...
        if counter % display_interval == 0:
            show_images(inpainted_image[0], input_occluded[0], target[0], f"Training (Batch {counter})")
...
def validate(model, gan, valid_dataset, valid_dataloader, device, criterion, criterion_gan, epoch, a, b, c, d):
    ...
    Inputs:
        model = Inpainting model
        gan = discriminator
        valid_datasatet = images that the model will use to be train with
        valid_dataloader = it creates the defined loader 
        device = CUDA if available
        criterion = to calculate L1, perceptual, and style loss 
        criterion_gan = to calculate adversarial loss
        a, b, c, d = hyperparameters of the total loss function
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(prog_bar):
            ...
            input_occluded = data[0].to(device) ###Occluded image
            target = data[1].to(device) ### Original image
            out_M = model(input_occluded) ### Model's prediction

            perceptual, style, content = criterion(out_M, target) ### Losses
            
            ### GAN
            real_labels = torch.ones_like(gan(target))     
            adversarial_loss = criterion_gan(gan(out_M),real_labels)
            
            perceptual,style, content = criterion(out_M,target)
            ...
            ### Comment if run from console. Meant to keep track of the reconstruction during training. It displays results in the GUI.    
            if counter % display_interval == 0:
                show_images(out_M[0], input_occluded[0], target[0], f"Valid (Batch {counter})")
